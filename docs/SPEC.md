...
- **PR1.6**: Broad-scope SKU surface deletion (D27). `018_drop_sku_columns.sql` drops six columns (`users.stripe_subscription_id`, `users.subscription_status`, `users.agent_name`, `users.brokerage`, `lookups.report_type`, `reports.matter_reference`); deletes `src/app/api/subscription/*`, `src/app/api/lookup/[id]/report-type/`, `src/app/subscribe/`, `src/lib/subscription.ts`; strips `attorneyReport` + `agentPlan` from `config.pricing`; removes the SKU-selector and matter-reference UIs from `results/[id]`, the subscription/profile block from `dashboard`, the agent fields from `OnboardingModal`, the attorney cover page and brokerage rendering from `lib/pdf.ts`, the subscription/attorney branches from `webhooks/stripe/route.ts` and `checkout/create/route.ts`, and the `report_type`/`matter_reference` Stripe metadata. Closes D20; partially closes D19. Subject to `migration-approved` label gate.
