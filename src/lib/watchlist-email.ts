...
export async function sendWatchlistAlert(
  ...
  const permitRows = newPermits
    ...
    .map(
      (p) => `
      <tr>
        <td style="padding: 8px 12px; font-family: monospace; font-size: 12px;">${p.recordNumber}</td>
        <td style="padding: 8px 12px; font-size: 12px;">${p.type}</td>
        <td style="padding: 8px 12px; font-size: 12px; font-weight: 600;">${p.status}</td>
        <td style="padding: 8px 12px; font-size: 12px;">${p.filedDate ?? "\u2014"}</td>
      </tr>`
    )
    .join("");
...
<body style="font-family: -apple-system, sans-serif; padding: 40px; background: #f8fafc;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; border: 1px solid #e5e7eb;">
    ...
    <div style="padding: 28px;">
      ...
      <a href="${reportUrl}" style="display: inline-block; padding: 12px 24px; background: #0f1f3d; color: white; text-decoration: none; border-radius: 8px; font-size: 14px; font-weight: 600;">
